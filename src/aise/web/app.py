...
class WebProjectService:
    ...
    def _execute_workflow_run(self, project_id: str, run_id: str, requirement: str) -> None:
        ...
        try:
            ...
            with self._lock:
                project = self.project_manager.get_project(project_id)
                if project is not None:
                    project._dynamic_plan_pending = True  # type: ignore[attr-defined]
    ...
    def _build_workflow_nodes(self, project: Project) -> list[dict[str, Any]]:
        ...
        # Plan is still being generated by background thread
        if getattr(project, "_dynamic_plan_pending", False):
            return [
                {
                    "name": "planning",
                    "tasks": [],
                    "agent_tasks": [
                        {
                            ...
                            "tasks": [
                                {
                                    "key": "ai_planner.generating_plan",
                                    "label": "AI is analyzing requirements and generating workflow...",
                                }
                            ],
                        }
                    ],
    ...
    def _expand_phase_subagents(
        ...
        _is_product = phase in ("requirements", "deep_product_workflow") or "deep_product_workflow" in phase
        if _is_product and "product_manager.deep_product_workflow" in task_keys:
            return [
                {
        ...
        _is_arch = phase in ("design", "deep_architecture_workflow") or "deep_architecture_workflow" in phase
        if _is_arch and "architect.deep_architecture_workflow" in task_keys:
            return [
                {
        ...
        _is_dev = phase in ("implementation", "deep_developer_workflow") or "deep_developer_workflow" in phase
        if _is_dev and "developer.deep_developer_workflow" in task_keys:
            return [
                {
                    "agent": "programmer[*]",
                    "tasks": [
